$(function(){
            ...
            function addlist(data){ 
                for(var i=0 ;i<data.length;i++){
                    for(var key in data[i]){
                        ...
                        console.log(data[i])
                        if(data[i]['gnum'] == '1'){
                            hmlstr='<p class=\"p-subtitlt\">'+data[i]['brief']+'</p>'
                        }
    
                    }
                    // bImage = obj[i]['bimages'].split('/');
                    // sImage =obj[i]['simages'].split('/');
                    // price =obj[i]['price'].split('/');
                    // name =obj[i]['name'].split('/');
                    // tz  =obj[i]['tz'].split('/');
                    // $('')
                }           
    ...
    goodlistFun.goodlist($('.gl-item'),$('.gl-options'))
})
